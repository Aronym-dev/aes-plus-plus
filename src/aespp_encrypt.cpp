		...
		for (i = 0; i < BLOCK_SIDE; i++) {
			for (j = 0; j < BLOCK_SIDE; j++) {
				...
				for (k = 0; k < BLOCK_SIDE; k++) {
					output[i][j] ^= AES_MATH::galoisMultiplication(mixColumnMatrix[i][k], state[k][j]);
				}
			}
		}

		// Copy output to the state
		std::memcpy(state, output, BLOCK_SIDE * BLOCK_SIDE * sizeof(unsigned char));
	}
